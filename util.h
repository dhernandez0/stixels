...
/*
__device__ __forceinline__ uchar4 scalarToVector(const uint32_t s) {
	uchar4 v;
	asm("mov.b32 {%0,%1,%2,%3}, %4;" : "=h"(v.x), "=h"(v.y), "=h"(v.z), "=h"(v.w) : "r"(s));
	return v;
}
__device__ __forceinline__ void scalarToVector(const uint32_t s, uint8_t *u1, uint8_t *u2, uint8_t *u3, uint8_t *u4) {
	asm("mov.b32 {%0,%1,%2,%3}, %4;" : "=h"(u1), "=h"(u2), "=h"(u3), "=h"(u4) : "r"(s));
}
*/
...
__inline__ __device__ uint8_t minu8_index4(int *min_idx, const uint8_t val1, const int dis,
		const uint8_t val2, const int dis2, const uint8_t val3, const int dis3, const uint8_t val4,
		const int dis4) {
	int min_idx1 = dis;
	uint8_t min1 = val1;
	if(val1 > val2) {
...
__inline__ __device__ uint8_t minu8_index8(int *min_idx, const uint8_t val1, const int dis,
		const uint8_t val2, const int dis2, const uint8_t val3, const int dis3, const uint8_t val4,
		const int dis4, const uint8_t val5, const int dis5, const uint8_t val6, const int dis6,
		const uint8_t val7, const int dis7, const uint8_t val8, const int dis8) {
...
template<typename T>
__inline__ __device__ T warpReduceAdd(T val) {
...
__inline__ __device__ int warpReduceMin(int val, int num) {
	for(int i = 1; i < num; i*=2) {
		val = min(val, shfl_xor_32(val, i));
	}
	return val;
}

__inline__ __device__ int blockReduceMin(int val, int warp_width) {
	...
	val = warpReduceMin(val, warp_width); //Final reduce within first warp
...
__inline__ __device__ int blockReduceMinIndex2(int *val, int idx) {
	...
	//read from shared memory only if that warp existed
	*val = (lane < blockDim.x / WARP_SIZE) ? shared_val[lane] : INT_MAX;
	idx = shared_idx[lane];
...
__inline__ __device__ int blockReduceMinIndex(int val, int idx) {
	...
	//read from shared memory only if that warp existed
	val = (lane < blockDim.x / WARP_SIZE) ? shared_val[lane] : INT_MAX;
	idx = (lane < blockDim.x / WARP_SIZE) ? shared_idx[lane] : INT_MAX;
